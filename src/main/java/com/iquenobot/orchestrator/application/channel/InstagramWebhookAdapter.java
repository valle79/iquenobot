...
@Slf4j
public class InstagramWebhookAdapter {

    private final ConversationOrchestrator orchestrator;

    public ProcessingResult processWebhook(String instanceId, InstagramWebhookDto payload) {
        if (payload.getEntry() == null || payload.getEntry().isEmpty()) {
            log.warn("Instagram webhook with no entries");
            return ProcessingResult.failure("NO_ENTRIES", "Webhook contains no entries", 0);
        }

        InstagramWebhookDto.Messaging firstMsg = payload.getEntry().get(0).getMessaging().get(0);
    ...
    private MessageType resolveMessageType(InstagramWebhookDto.Messaging msg) {
    ...
    private String extractMediaUrl(InstagramWebhookDto.Messaging msg) {
